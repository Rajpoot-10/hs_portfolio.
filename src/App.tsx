...
function CaseStudy() {
  ...
  return <main id="main" tabIndex={-1} className="container case-study"><Link to="/#projects" className="back-link"><ArrowLeft size={16}/> Back to selected work</Link><p className="eyebrow">PROJECT STUDY / {project.category.toUpperCase()}</p><h1>{project.title}</h1><p className="case-intro">{project.description}</p><ul className="tags">{project.stack.map(tech => <li key={tech}>{tech}</li>)}</ul><div className="case-external">{getProjectRepositories(project).map(repository => <ExternalLink key={repository.url} href={repository.url} className="button button-outline">{repository.label} <Github size={16}/></ExternalLink>)}<ExternalLink href={project.demo} className="button button-primary">Live demo <ArrowUpRight size={16}/></ExternalLink></div><ProjectPreview project={project} large/>
