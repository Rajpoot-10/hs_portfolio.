...
import { assetUrl, education, emailUrl, expertise, featuredProjectOrder, filters, findProject, getProjectRepositories, orderedProjects, profile, projects, webUrl } from './data/content';
...
function PageEffects() {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      if (target) { target.scrollIntoView({ behavior: initial.current ? 'auto' : 'smooth', block: 'start' }); if (!initial.current) target.focus({ preventScroll: true }); }
...
function SectionHeading({ number, label, title, children }: { number: string; label: string; title: string; children?: ReactNode }) {
  return <div className="section-heading" data-reveal><div><p className="eyebrow"><span>{number} /</span> {label}</p><h2>{title}</h2></div>{children && <p className="section-description">{children}</p>}</div>;
}
function ProjectActions({ project }: { project: Project }) {
  return <div className="project-actions"><Link to={`/projects/${project.slug}`} className="text-link">View case study <ArrowUpRight size={17} /></Link>{getProjectRepositories(project).map(repository => <ExternalLink key={repository.url} href={repository.url} className="subtle-link">{repository.label} <Github size={15} /></ExternalLink>)}<ExternalLink href={project.demo} className="subtle-link">Live demo <ArrowUpRight size={15} /></ExternalLink></div>;
}
function ProjectCard({ project, featured, index }: { project: Project; featured: boolean; index: number }) {
  return <article className={featured ? `featured-project featured-${index}` : 'compact-project'} data-testid="project-card" data-reveal data-reveal-delay={(index % 2) * 65}>
...
function CaseStudy() {
  ...
  return <main id="main" tabIndex={-1} className="container case-study"><Link to="/#projects" className="back-link"><ArrowLeft size={16} /> Back to selected work</Link><p className="eyebrow">PROJECT STUDY / {project.category.toUpperCase()}</p><h1 data-reveal>{project.title}</h1><p className="case-intro">{project.description}</p><ul className="tags">{project.stack.map(tech => <li key={tech}>{tech}</li>)}</ul><div className="case-external">{getProjectRepositories(project).map(repository => <ExternalLink key={repository.url} href={repository.url} className="button button-outline">{repository.label} <Github size={16} /></ExternalLink>)}<ExternalLink href={project.demo} className="button button-primary">Live demo <ArrowUpRight size={16} /></ExternalLink></div><ProjectPreview project={project} large />
    <div className="case-body"><aside><span className="eyebrow">IN THIS STUDY</span><nav aria-label="Case study sections">{['Overview', 'Problem', 'Approach', 'Features', 'Takeaways', 'Next steps'].map(item => <a key={item} href={`#${item.toLowerCase().replace(' ', '-')}`}>{item}<ChevronRight size={13} /></a>)}</nav></aside><div className="case-sections"><section id="overview" data-reveal><p className="eyebrow">01 / CONTEXT</p><h2>Overview</h2><p>{project.description}</p></section><section id="problem" data-reveal><p className="eyebrow">02 / THE QUESTION</p><h2>The problem</h2><p>{project.problem}</p></section><section id="approach" data-reveal><p className="eyebrow">03 / THE SYSTEM</p><h2>Approach & architecture</h2><p>{project.approach}</p><div className="architecture" aria-label="Conceptual architecture">{project.architecture.map((step, index) => <div key={step}><span>{String(index + 1).padStart(2, '0')}</span>{step}{index < project.architecture.length - 1 && <ArrowDown size={15} />}</div>)}</div><p className="caption">Conceptual flow based on the project’s described tools and features.</p></section><section id="features" data-reveal><p className="eyebrow">04 / CAPABILITIES</p><h2>Features</h2><ul className="feature-list">{project.features.map(feature => <li key={feature}><Check size={17} />{feature}</li>)}</ul></section><section id="takeaways" data-reveal><p className="eyebrow">05 / REFLECTION</p><h2>Technical takeaways</h2><p className="caption">Design considerations, rather than measured performance claims.</p>{project.considerations.map(point => <p key={point}>{point}</p>)}</section><section id="next-steps" data-reveal><p className="eyebrow">06 / LOOKING AHEAD</p><h2>Limitations & next steps</h2><p>These are proposed areas for further validation and development.</p><ul className="next-steps">{project.nextSteps.map(step => <li key={step}>{step}</li>)}</ul></section></div></div>
